...
# Mower: Arduino Software Design Description
...
The modules that can be found are:
  ...
  * `autonomous.ino` (code related to the mower moving in an autonomous way)
  * `diagnostics.ino` (self-diagnostics for higher requirement, checks the vital parts of the system)
  * `encoder.ino` (handling the pulses generated by the encoders and how to retrieve the data)
  * `gyro.ino` (handles gyroscope data used during localization)
  * `joystick.ino` (not used, planned to be used in conjunction with a joystick in the app)
  * `led.ino` (LED functionality, found on the Arduino board)
  * `localization.ino` (all dead-reckoning functionality, stores x- and y coordinate in centimeters)
...
The loop checks whether it should update (through the serial communication).
The 'true' here means that if there is any communication, try to read it and then acknowledge it back to the Pi.
...
The self-driving/autonomous code can be found within this file.
There is primarily one function that runs all of the autonomous driving but other functions are found within this main 'tick-function'.
The concept is rather simple:
If we have not collided with anything or driven over the boundary, continuously send our coordinates to the backend and drive forward.
...
Thanks to this, the encoder data can be used to fulfil the localization requirements, making it possible to calculate the 'local' coordinates based on how much the motors have rotated.
...
The encoders mentioned generate the data needed to know how far the mower has gone but not in what direction.
This is why the gyro is used within the mower.
The gyroscope's Z-axis data circles within -180 and 180, where 0 is straight ahead and -180/180 is when the mower is directed backwards.
-90 is 90 degrees to the left and 90 is 90 degrees to the right.
The `gyroAtStart` and `gyroAtEnd` are parameters used to remember where the mower started from and where it ended, making it possible to calculate the average gyro value from one movement, since that lowers the error margin.
...
A single global variable is used when either getting, setting or resetting the direction in which the mower should go, reducing the risk of accidentally moving is some direction by hard-coded functions.


The single tick-function found in this file is a simple move-function depending on what direction is received from the app via Bluetooth.
...
How the motors are controlled depending on the speed and direction is done by the following function:
...
The Arduino simply sends back either the message followed by an exclamation mark or a question mark, depending on if the Arduino acknoweleged the message or not.
However, if the Arduino receives some type of message it does not support, it sends back the message followed by a question mark: `"SOMEBADSTRING?"`.
...
It is run continuously within the main loop.
